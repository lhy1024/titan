...
namespace rocksdb {
namespace titandb {
DigHoleJob::DigHoleJob(
    TitanDBOptions titan_db_options, const EnvOptions &env_options, Env *env,
    TitanCFOptions titan_cf_options, std::function<bool()> IsShutingDown,
...
Status DigHoleJob::Exec(BlobFileMeta *input) {
  ...
  for (record_iter->SeekToFirst(); record_iter->Valid();) {
    ...
    const uint64_t hole_end =
        (record_end + block_size_ - 1) / block_size_ * block_size_;
    bool enable_hole = true;
    ...
    if (enable_hole && record_iter->status().ok()) {
