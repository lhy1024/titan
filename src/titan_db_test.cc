...
class TitanDBTest : public testing::Test {
 public:
  ...
  void VerifyBlob(uint64_t file_number,
    ...
    std::unique_ptr<PosixRandomRWFile> rw_file;
...
TEST_F(TitanDBTest, FileLifetime) {
  // a blob file first be dig hole then be gc
<<<<<<< HEAD
  options_.disable_background_gc = true;
=======
  options_.disable_background_gc = false;
>>>>>>> 04efbfc75a4c740099a956b87b854fc65af40404
  ...
  // delete some keys to trigger dig hole but not gc
  for (int i = 1; i < 201; i++) {
...
<<<<<<< HEAD
    ASSERT_TRUE(f.second->file_size() > files_2[f.first]->file_size());
=======
...
>>>>>>> 04efbfc75a4c740099a956b87b854fc65af40404
  ...
  for (auto& f: files_1) {
    // after gc the old blob files was deleted
    ASSERT_TRUE(files_3.find(f.first) == files_3.end());
  }

  Close();
}

TEST_F(TitanDBTest, PickFileToGCAndFS) {
<<<<<<< HEAD
  options_.disable_background_gc = true;
=======
  options_.disable_background_gc = false;
>>>>>>> 04efbfc75a4c740099a956b87b854fc65af40404
  options_.merge_small_file_threshold = 100 * options_.min_blob_size;
  options_.free_space_threshold = 10 * options_.min_blob_size;
  Open();
  ...
  auto old_files = blob.lock()->TEST_GetAllFiles();
  ASSERT_EQ(old_files.size(), 2);

  // delete few of keys to trigger dig hole but not gc
  for (int i = 1; i < 201; i++) {
    db_->Delete(WriteOptions(), GenKey(i));
  }
  Flush();

  // delete many of keys to trigger gc
  ...
  auto new_files = blob.lock()->TEST_GetAllFiles();
  ASSERT_EQ(new_files.size(), 2);
  
  bool had_dighole = false;
  bool had_gc = false;
  for (auto& old_file: old_files) {
    if (new_files.find(old_file.first) != new_files.end()) {
      // blob file that had been dig hole
<<<<<<< HEAD
      ASSERT_TRUE(old_file.second->file_size() > new_files[old_file.first]->file_size());
=======
      ASSERT_TRUE(old_file.second->real_file_size() > new_files[old_file.first]->real_file_size());
>>>>>>> 04efbfc75a4c740099a956b87b854fc65af40404
      had_dighole = true;
    } else {
      // new blob file that generated by gc
      had_gc = true;
    }
  }
  ASSERT_TRUE(had_dighole && had_gc);
